...
class EarlyStopping:
    def __init__(self, patience=40, min_delta=1e-5, path='best_model.pth'):
...
#Weighted MSE loss function to emphasize certain joints
...
# WEIGHTED LOSS: [J1, J2, J3, J4, J5, J6]
JOINT_WEIGHTS = torch.tensor([1.0, 1.0, 1.0, 2.5, 1.0, 1.0], dtype=torch.float32).to(DEVICE)

#Starting Multi-start training loop and logging
with open(LOG_FILE, "a") as log_f:
    log_f.write(f"{datetime.now()} - Multi-start session ({NUM_RESTARTS} initializations) on Split-GRU su {DEVICE}\n")
    log_f.write(f"Configuration: SEQ_LEN={SEQ_LEN}, LOW={HIDDEN_LOW}, HIGH={HIDDEN_HIGH}, LR={LR}\n\n")
...
final_model_path = "best_robot_split_gru_weighted.pth"
...
for run in range(NUM_RESTARTS):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for b_X, b_y in train_loader:
            ...
            optimizer.zero_grad()
            output = model(b_X)
            loss = weighted_mse_loss(output, b_y, JOINT_WEIGHTS)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for bv_X, bv_y in val_loader:
                v_loss += weighted_mse_loss(model(bv_X.to(DEVICE)), bv_y.to(DEVICE), JOINT_WEIGHTS).item()

        if epoch % 20 == 0:
            print(f"  > Epoch {epoch} - Current Loss: {loss.item():.6f}")
    ...
    log_entry = f"{datetime.now()} - Run {run+1}/{NUM_RESTARTS} - Best Val Loss: {early_stopping.best_loss:.6f} - Time: {duration:.2f}s\n"
    print(log_entry)
    with open(LOG_FILE, "a") as log_f:
...
print(f"\nTotal R2 mean: {np.mean(r2_all):.2f}%")